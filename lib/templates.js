...
function preencherTemplate(template, cliente, clienteId) {
  const diasRestantes = Math.ceil(
    (cliente.vencimento - Date.now()) / (1000 * 60 * 60 * 24)
  );
  const link = clienteId ? `${process.env.APP_URL}/meu-plano.html?id=${clienteId}` : '';
...
const TEMPLATES_PADRAO = {
  ...
  msg3diasVencido:
    'Olá {nome}! Seu plano está vencido desde {data_vencimento} e o pagamento está pendente. Valor: R$ {valor_plano}. Vamos regularizar hoje pra reativar o acesso?',
  msgVencido:
    'Olá {nome}! Seu plano venceu em {data_vencimento} e está com pagamento pendente. Valor: R$ {valor_plano}. Vamos regularizar hoje pra não perder o acesso?',
  msgManual:
  ...
  msgTesteAcabando:
    'Olá {nome}! Seu teste está acabando em breve. Gostou? Fala com a gente pra virar cliente oficial e não perder o acesso!',
  msgTesteAcabou:
    'Olá {nome}! Seu período de teste terminou. Gostou da experiência? Fala com a gente agora pra ativar seu plano oficial!',
  msgBemVindoOficial:
    'Olá {nome}! Seu plano foi ativado com sucesso ✅\nVencimento: {data_vencimento}\nValor: R$ {valor_plano}\nSeja bem-vindo(a)!',
