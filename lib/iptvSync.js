...
async function sincronizarComPainelIPTV(id, cliente, templates) {
  ...
  const corpo = preencherTemplate(templates.comprovanteRenovacao || '', clienteAtualizado, id);
  ...
  if (cliente.email) {
    try {
      await resend.emails.send({
        ...
        to: cliente.email,
        subject: preencherTemplate(templates.emailAssunto || 'Seu plano foi renovado!', clienteAtualizado, id),
        text: corpo,
      });
    } catch (err) {
