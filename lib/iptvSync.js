...
const { consultarContaXtream, consultarContaMusica } = require('./xtream');
const { Resend } = require('resend');

const resend = new Resend(process.env.RESEND_API_KEY);

// Confere se o cliente foi renovado direto no painel de origem (IPTV via
// link M3U, ou Música via usuário/senha no domínio fixo). Se a data lá
// estiver mais nova que a nossa, atualiza sozinho e dispara o mesmo aviso
// de renovação que o botão manual dispara (push + e-mail). Retorna o
// cliente atualizado + o texto do comprovante (pra quem chamou poder
// abrir o WhatsApp também).
async function sincronizarComPainelIPTV(id, cliente, templates) {
  let resultado;

  if (cliente.musica) {
    if (!cliente.usuario || !cliente.senha) return null;
    const configSnap = await db.ref('config/musicaApiBase').once('value');
    const apiBase = configSnap.val();
    resultado = await consultarContaMusica(cliente.usuario, cliente.senha, apiBase);
  } else {
    if (!cliente.m3uLink) return null;
    resultado = await consultarContaXtream(cliente.m3uLink);
  }
  ...
  const corpo = preencherTemplate(templates[chaveComprovante] || '', clienteAtualizado, id);
  ...
  if (cliente.email) {
    try {
      await resend.emails.send({
        ...
        to: cliente.email,
        subject: preencherTemplate(templates[chaveAssunto] || 'Seu plano foi renovado!', clienteAtualizado, id),
        text: corpo,
      });
    } catch (err) {
